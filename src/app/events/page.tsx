              ...
              <ul className="space-y-16 z-10 relative">
                {timeline.map((event, idx) => (
                  <li key={idx} className="flex flex-col md:flex-row items-center md:items-start md:space-x-8 group">
                    ...
                    <div className="flex-1 bg-white/80 dark:bg-neutral-900/80 rounded-3xl p-12 shadow-2xl border-4 border-orange-300 dark:border-orange-900 backdrop-blur-lg min-h-[200px] flex flex-col justify-center items-center">
                      <h3 className="font-orbitron text-3xl md:text-4xl font-semibold text-orange-600">{event.title}</h3>
                    </div>
                  </li>
                ))}
              </ul>
